...
{
    public:
        cDiMeshBuffer(Di::cDevice* d, Di::cFlags* f)
        {
            dev = d;
            flags = f;
            ...
            int R = 7;
            vector3di displace = vector3di(R,R,R);
            ...
            int zr = 16;
            int yr = 8;
            vector3di displace = vector3di(xr,yr,zr);
            for (int x=0; x < PARAM_CHUNK_SIZE; x++)
            {
                for (int z=0; z < PARAM_CHUNK_SIZE; z++)
                {
        ...
        */
        ...
        void blockDel(vector3di blockPos, bool generatePlanes=true, bool N=true, bool S=true, bool W=true, bool E=true, bool U=true, bool D=true)
        {
            if (DEBUG_INFO)
            {
                ostringstream ss; ss << "Trying to delete block on XYZ = " << blockPos.X << " / " << blockPos.Y << " / " << blockPos.Z << " :";
                dev->logger->write(ss.str().c_str());
                cout << ss.str();
            }
            ...
            {
                ostringstream ss; ss << "ERROR: block coordinates < 0";
                dev->logger->write(ss.str().c_str());
                cout << ss.str();
                return;
            }
            ...
            {
                ostringstream ss; ss << "ERROR: block does not exist";
                dev->logger->write(ss.str().c_str());
                cout << ss.str();
                return;
            }
        ...
        bool generatePlaneMesh(vector3di pos)
        {
            if (DEBUG_INFO)
            {
                ...
                dev->logger->write(ss.str().c_str());
                cout << ss.str();
            }                                                                                                              //N  S  W  E  U  D   N  S  W  E  U  D
            if (matrix[pos.X-1][pos.Z][pos.Y] == 1 && pos.X != 0) dublicateTo(createPlaneMesh(vector3di(pos.X-1,pos.Y,pos.Z),0,-1,-1,-1,-1,-1),pos.X-1,pos.Y,pos.Z);
            if (matrix[pos.X+1][pos.Z][pos.Y] == 1 && pos.X != (PARAM_CHUNK_SIZE-1)) dublicateTo(createPlaneMesh(vector3di(pos.X+1,pos.Y,pos.Z),-1,0,-1,-1,-1,-1),pos.X+1,pos.Y,pos.Z);
            if (matrix[pos.X][pos.Z-1][pos.Y] == 1 && pos.Z != 0) dublicateTo(createPlaneMesh(vector3di(pos.X,pos.Y,pos.Z-1),-1,-1,-1,0,-1,-1),pos.X,pos.Y,pos.Z-1);
            if (matrix[pos.X][pos.Z+1][pos.Y] == 1 && pos.Z != (PARAM_CHUNK_SIZE-1)) dublicateTo(createPlaneMesh(vector3di(pos.X,pos.Y,pos.Z+1),-1,-1,0,-1,-1,-1),pos.X,pos.Y,pos.Z+1);
            if (matrix[pos.X][pos.Z][pos.Y-1] == 1 && pos.Y != 0) dublicateTo(createPlaneMesh(vector3di(pos.X,pos.Y-1,pos.Z),-1,-1,-1,-1,0,-1),pos.X,pos.Y-1,pos.Z);
            if (matrix[pos.X][pos.Z][pos.Y+1] == 1 && pos.Y != (PARAM_CHUNK_HEIGHT-1)) dublicateTo(createPlaneMesh(vector3di(pos.X,pos.Y+1,pos.Z),-1,-1,-1,-1,-1,0),pos.X,pos.Y+1,pos.Z);
            return true;
        }
